...
for (const f of compFiles) {
  ...
  // Clara's bust and avatar frames arrive pre-cropped against the manifest's
  // focal anchor, so no component crops with CSS any more: every frame is
  // contain-fit. (The Lumi-era BUST_FOCAL table stays exported in
  // lib/character.ts for the store's cosmetic artwork.)
...
if (existsSync(illusPath)) {
  ...
  assert(/aria-hidden/.test(src) && /meaningful\?/.test(src),
...
assert(/CharacterIllustration/.test(homeSrc),
  "dashboard hero renders the guide through the safe character illustration");
assert(!/LumiDepth|<canvas/.test(homeSrc),
  "dashboard hero never uses the WebGL depth renderer that duplicates Lumi");
