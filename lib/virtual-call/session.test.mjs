...
// Pronunciation: present whenever the service actually scored the audio —
// which is every spoken turn now that unscripted assessment is used, not only
// repeat-after-me retries. Still never inferred from a transcript.
eq(summarizePronunciation(turnsForReport), undefined,
  "no pronunciation summary when nothing was scored at all");
{
  // A free-conversation turn carries a score with no target: Azure graded what
  // she actually chose to say. It must count.
  const unscripted = [
    { index: 0, transcript: "I work as a designer.", at: 1, correction: null,
      pronunciation: { score: 78, worstWord: "designer" } },
    { index: 1, transcript: "Yesterday I went home.", at: 2, correction: null,
      pronunciation: { score: 88, target: "Yesterday I went home.", worstWord: "yesterday" } },
  ];
  const sum = summarizePronunciation(unscripted);
  eq(sum?.scored, 2, "an unscripted turn counts toward the pronunciation summary");
  eq(sum?.averageScore, 83, "the average spans scripted and unscripted turns alike");
}
const withPron = [
