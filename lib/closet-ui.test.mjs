...
check(
  source.includes('<ul className="mt-4 grid grid-cols-2 gap-3" aria-label="City Remix">')
    && source.includes("<li key={cosmetic.id}>")
    && source.includes('aria-pressed={chosen}'),
  "the six-look picker exposes valid list and button selection semantics",
);
...
check(!source.includes("<main"), "Closet relies on the app's single main landmark instead of nesting another main");
