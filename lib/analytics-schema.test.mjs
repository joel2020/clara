...
// Contextual visuals may report only their bounded route, object, asset, and
// fallback handles. These literals deliberately do not derive from the schema:
// changing a visual event's required shape must break this contract.
const VISUAL_EVENT_PROPS = {
  visual_topic_shown: { topicId: "coffee-order", lessonId: "coffee-basics" },
  visual_object_selected: { topicId: "coffee-order", lessonId: "coffee-basics", objectId: "menu" },
  visual_object_replay: { topicId: "coffee-order", lessonId: "coffee-basics", objectId: "menu" },
  visual_discovery_complete: { topicId: "coffee-order", lessonId: "coffee-basics" },
  visual_to_speaking: { topicId: "coffee-order", lessonId: "coffee-basics" },
  visual_fallback: { fallbackClass: "topic-missing" },
  visual_asset_loaded: { assetClass: "environment", loadBand: "under-250" },
};

for (const [type, required] of Object.entries(VISUAL_EVENT_PROPS)) {
  truthy(ANALYTICS_EVENT_TYPES.includes(type), `${type} is in the closed union`);
  eq(validateEvent({ type, props: required }).props, required, `${type} keeps its bounded required properties`);
  eq(
    validateEvent({ type, props: { ...required, harmlessFutureField: true } }).props,
    required,
    `${type} drops a harmless unknown property`,
  );
  for (const denied of ["transcript", "studentName", "rawPayload"]) {
    throws(
      () => validateEvent({ type, props: { ...required, [denied]: "secret" } }),
      `${type} rejects a ${denied} property`,
    );
  }
  for (const key of Object.keys(required)) {
    if (!key.endsWith("Id")) continue;
    throws(
      () => validateEvent({ type, props: { ...required, [key]: "she said hello to me" } }),
      `${type} rejects a sentence in ${key}`,
    );
  }
}
