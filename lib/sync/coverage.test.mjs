...
const DEVICE_ONLY = {
  ...
  virtualCalls:
    "one end-of-call report per Virtual Call, plus the transcript of what she said WHEN she opts into keeping it (Settings.callTranscriptRetention). Device-only for two reasons: the live schema has no virtual_calls table to write to, and a kept transcript is a per-device privacy choice that must not be silently uploaded. Syncing the report later means adding the table AND keeping the transcript out of the payload — the notice at /privacidad promises the conversation stays on this device.",
};
...
for (const fn of exportedPushers) {
  check(callers.includes(fn), `${fn} is called somewhere (defined but unused is the same as broken)`);
}
