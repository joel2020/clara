...
const migrationV9Path = "supabase/migration-v9-closet-unlocks.sql";
const migrationV9Sql = existsSync(migrationV9Path) ? readFileSync(migrationV9Path, "utf8") : "";
...
check(migrationV9Sql.length > 0, "the Closet unlock migration exists");
for (const column of ["completed_daily_sessions", "unlocked_milestones"]) {
  check(
    hasColumn(playerStatsBody, column) && normalized(migrationV9Sql).includes(`add column if not exists ${column}`),
    `authoritative schema and Closet migration define ${column}`,
  );
}
for (const fragment of [
  "create or replace function public.merge_player_closet_progress",
  "greatest(completed_daily_sessions, p_completed_daily_sessions)",
  "grant execute on function public.merge_player_closet_progress",
]) {
  check(
    normalizedCurrentSchemaSql.includes(normalized(fragment)) && normalized(migrationV9Sql).includes(normalized(fragment)),
    `authoritative schema mirrors Closet merge contract: ${fragment}`,
  );
}
...
check(src.includes('rpc("merge_player_closet_progress"'), "player sync uses the monotonic Closet merge RPC");
