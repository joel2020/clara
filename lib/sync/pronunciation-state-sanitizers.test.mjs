...
assert(player?.completedDailySessions === 0 && player.unlockedMilestones.length === 0, "legacy player retries receive safe Closet defaults");
const closetPlayer = playerOutboxPayload({
  ...DEFAULT_PLAYER,
  completedDailySessions: 7,
  unlockedMilestones: ["first-passed-interview-call"],
});
assert(closetPlayer?.completedDailySessions === 7, "the lifetime daily-session count crosses the retry boundary");
assert(closetPlayer?.unlockedMilestones[0] === "first-passed-interview-call", "earned Closet milestones cross the retry boundary");
